...
public class Network {
    ...
    public void sendMessage(String message) throws IOException {
        try {
            ...
            System.out.println("Не удалось отправит ьсообщение на сервер");
            throw e;
        }
    }

    public void waitMessages(Consumer<String> messageHendler) {
        Thread thread = new Thread(() -> {
            while (true) {
                try {
                    String message = this.socketInput.readUTF();
                    messageHendler.accept(message);
                } catch (IOException e) {
                    e.printStackTrace();
                    System.out.println("Не удалось прочитать сообщения от сервера");
                    break;
                }
            }
        });
